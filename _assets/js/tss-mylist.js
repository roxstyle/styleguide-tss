...
function accountMenu( ){
	$(".account .nav-lists nav.sections>ul>li>a").click( function() {
		...
		if ((active).hasClass("user-profile")) {
			...
			$(".account #main > #user section").show( );
		}
...
function customlistTemplate() {
	var $options = $('.list-options .sl-list'),
		$template = $('.list-template .sl-list'),
		$bye = $(".list-template .sl-list .delete");
	var totalItems = 0;
	...
	$('li', $options).draggable({
		...
		appendTo: ".custom-lists > .list-template",
		helper: "clone",
		revert: "invalid",
		zIndex: 99999,
		stop: function( event, ui ) {
			//ui.helper.empty();
			$(this).addClass("ui-state-disabled").draggable("option", "disabled", true );
		}
	});

	//template is droppable
	$template.droppable({
		accept: ".list-options .sl-list > li.sl-item",
		tolerance: 'pointer',
	...
	function handleDrop() {
			...
			var itemtemp = ui.draggable.data("id");

			if ( itemid === itemtemp ) {
				$(itemtemp).removeClass('ui-draggable').addClass("ui-state-disabled").draggable("option", "disabled", true );
			}

	}

	function removeList($bye) {
		...
		$bye.click(function() {
			$(this).parent("li").remove();
				...
				if ( $(this).parent("li").data("id") ===  $(opt) ) {
					$(opt).removeClass('ui-state-disabled').addClass('ui-draggable').draggable("option", "disabled", true );
				}
	...
	if (totalItems > 8) {
		$('.list-options .sl-list li').draggable("option", "disabled", true );
	}
...
//edit a custom list
function customEdit() {
	$(".list-details li button.edit").on("click", function() {
...
function customDelete() {
	$(".list-details li .delete").on("click", function() {

		var id = $(this).closest("li").data("id");
		var temp = $(this).parents('.custom-lists').find(".list-template .sl-item").data("id");
			if ( id === temp ) {
				$(id).removeClass('ui-state-disabled', 'ui-draggable-disabled').addClass('ui-draggable').draggable("option", "disabled", false );
			}
...
function mylistsFilter() {
	...
	function mylistsDefault() {
		...
		$(list).removeClass('active').find('> .ml-type').removeClass('active').end()
			.find('> .my-list-edit').removeClass('expanded').prop('checked', false).end()
			.find('> .list-details').removeClass('expanded');
		$(f).find('> .custom-lists').removeClass('active').end().find('> .list-name').removeClass('active');
	}
